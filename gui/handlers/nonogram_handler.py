...
class NonogramHandler:
    ...
    def load_file(self, path: str) -> None:
        ...
        if filetype == 'lp':
            ...
            height, width = len(row_hint_lines), len(col_hint_lines)

            for row_index, line in enumerate(row_hint_lines, start=1):
                try:
                    hints = list(map(int, line.split()))
                except ValueError:
                    raise Warning(f"Invalid hint in row {row_index}")
            ...
            for col_index, line in enumerate(col_hint_lines, start=1):
                try:
                    ...
                    raise Warning(f"Invalid hint in column {col_index}")
                col_hints.append(hints)
        else:
            raise Warning("File Type not Supported")
